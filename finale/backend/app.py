...
#     # Load sentence-transformers embedder (used for role-based MMR queries)
#     try:
#         # assumes a cached model dir exists at ./cached_model; fallback to 'all-MiniLM-L6-v2' if not present
#         if Path("./cached_model").exists():
#             embedder = SentenceTransformer("./cached_model")
#             logger.info("SentenceTransformer loaded from ./cached_model")
#         else:
#             logger.info("Cached model not found. Loading default 'all-MiniLM-L6-v2' from internet (may take time)...")
#             embedder = SentenceTransformer("all-MiniLM-L6-v2")
#             logger.info("SentenceTransformer default model loaded")
#     except Exception as e:
#         logger.error(f"Failed to load SentenceTransformer: {e}")
#         embedder = None

# # --- existing PDF-processing helpers (copied from your previous app.py) ---
# def is_bullet_point(text):
#     text = text.strip()
#     bullet_patterns = [
#         r'^[•·▪▫▬►‣⁃]\s*', r'^\*\s+', r'^-\s+', r'^—\s+', r'^–\s+',
...
# # -------------------------
# # New: MMR + role-query logic
# # -------------------------
# def mmr(query_emb, sections, lambda_param=0.72, top_k=5):
#     """
#     sections: list of dicts each with a key 'embedding' (numpy array or list)
#     returns: selected indices and sim_q list
#     """
#     if not sections:
#         return [], []

#     selected, remaining = [], list(range(len(sections)))
#     # compute sim_q (similarity of each section to query)
#     sim_q = [util.cos_sim(query_emb, s['embedding']).item() for s in sections]
#     # pairwise doc sim
#     sim_doc = [[util.cos_sim(sections[i]['embedding'], sections[j]['embedding']).item() for j in range(len(sections))] for i in range(len(sections))]
...
#             mmr_scores = []
#             for idx in remaining:
#                 # max similarity of candidate to already selected
#                 max_sim = max(sim_doc[idx][j] for j in selected) if selected else 0
#                 score = lambda_param * sim_q[idx] - (1 - lambda_param) * max_sim
#                 mmr_scores.append(score)
#             chosen_rel_index = int(np.argmax(mmr_scores))
...
#     Accepts JSON:
#     {
#       "persona": { "role": "<role string>" } OR "persona": "<role string>",
#       "job_to_be_done": { "task": "<task string>" } OR "job_to_be_done": "<task string>",
#       "documents": [
#          { "filename": "<serverFilename>", "outline": { "title": "...", "outline": [ { "level": "...", "text":"...", "page": <n> }, ... ] } },
#          ...
#       ]
#     }
#     Returns JSON in the format of challenge1b_output.json
#     """
#     try:
#         data = request.get_json(force=True)
#         if data is None:
...
#         # Build sections from supplied outlines (do NOT re-extract headings)
#         section_data = []
#         for doc in documents:
#             filename = doc.get('filename') or doc.get('serverFilename') or doc.get('name')
#             outline_obj = doc.get('outline') or {}
#             out_list = outline_obj.get('outline') if isinstance(outline_obj, dict) else None
#             if not out_list:
#                 # try if outline is an array directly
#                 if isinstance(doc.get('outline'), list):
#                     out_list = doc.get('outline')
#             if not out_list:
#                 # no headings from this doc — skip
#                 continue

#             for item in out_list:
#                 # expected item shape: { level, text, page }
#                 text = item.get('text') if isinstance(item, dict) else str(item)
#                 page = item.get('page') if isinstance(item, dict) else None
#                 heading = text
#                 full_text = heading  # since we don't have body paragraphs on frontend, keep heading only
#                 emb = embedder.encode(full_text, normalize_embeddings=True)
#                 section_data.append({
#                     'Document': filename,
#                     'Page': page if page is not None else -1,
