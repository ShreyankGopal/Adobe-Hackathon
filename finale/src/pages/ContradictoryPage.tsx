  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-red-50 to-orange-100 p-4">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-4 h-[calc(100vh-120px)]">
        {/* Left Panel */}
        <div className="lg:col-span-1 bg-slate-900/90 backdrop-blur-sm rounded-xl shadow-xl border border-slate-700 p-4 flex flex-col h-[calc(100vh-2rem)]">
          {/* Header */}
          <div className="flex-shrink-0">
            <h2 className="text-lg font-bold text-slate-100 mb-1">Contradiction Analysis</h2>
            <p className="text-xs text-slate-400 mb-3">Conflicting information found.</p>
          </div>

          {/* Original Text */}
          <div className="mb-4 flex-shrink-0">
            <label className="block text-xs font-semibold text-slate-300 mb-2">Original Text</label>
            <div className="max-h-32 border border-slate-700 rounded-lg p-3 text-sm bg-slate-800 overflow-y-auto text-slate-200">
              {selectedText}
            </div>
          </div>
        ...
        {/* Left Panel (Results) */}
        <div className="lg:col-span-3 bg-slate-900/90 backdrop-blur-sm rounded-xl shadow-xl border border-slate-700 p-4 flex flex-col h-[calc(90vh-2rem)]">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-xl font-bold text-slate-100">Contradiction Results</h3>
            <button
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
                {result.extracted_sections.map((s: any, idx: number) => (
                  <div
                    ...
                    className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-lg p-3 hover:shadow-md hover:border-slate-500 cursor-pointer transition-colors"
                  >
                    <div className="text-xs text-slate-400">Document</div>
                    <div className="font-semibold text-sm mb-2 text-slate-200 truncate">{s.document}</div>
                    <div className="text-xs text-slate-400">Section</div>
                    <div className="text-sm text-slate-300 mb-2 line-clamp-2">{s.section_title}</div>
            ...
            <div>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                {result?.subsection_analysis?.map((sa: any, idx: number) => (
                  <div
                    key={idx}
                    className="bg-slate-800 border border-slate-700 rounded-lg p-4 shadow-sm hover:shadow-md hover:border-slate-500 transition-colors"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <div className="text-xs text-slate-400 font-medium">Document</div>
                        <div className="font-semibold text-slate-200 text-sm">{sa.document}</div>
                      </div>
                      <div className="text-xs text-slate-500 bg-slate-700/50 px-2 py-1 rounded">
                        Page {sa.page_number}
                      </div>
                    </div>
                    <div className="text-xs text-slate-400 font-medium mb-1">Analysis</div>
                    <div className="text-sm text-slate-300 leading-relaxed">{sa.refined_text}</div>
                  </div>
                ))}
              </div>
            </div>
