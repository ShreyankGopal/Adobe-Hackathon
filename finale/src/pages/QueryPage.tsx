...
//     try {
//       // Step 1: Call role_query for sections
...
//       // Simulate step progression with 3-second intervals
//       setLoadingMessage('Summarizing results...');
//       await new Promise(resolve => setTimeout(resolve, 3000));
      
//       setLoadingMessage('Getting interesting facts...');
//       await new Promise(resolve => setTimeout(resolve, 3000));
...
const RoleQueryPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    setResult(null);
    // Clear old query page insights on new submission
  ...
  // Memoize the generation of the text payload for the RightPanel to avoid re-computation
  ...
  return (
    <div className="min-h-screen bg-[#030303]">
            ...
            <div className="lg:col-span-1 bg-[#2A0A2A]/90 backdrop-blur-sm rounded-xl shadow-xl border border-white/10 p-4 flex flex-col max-h-full overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-3 flex-shrink-0">
                    ...
                    <div>
                        ...
                        <input type="number" className="w-full border border-gray-700 rounded-lg px-3 py-2 text-sm bg-[#1a1a1a] text-white" value={ranks} onChange={(e) => setRanks(e.target.value)} placeholder="e.g., 5" min="0" />
                    </div>
                ...
                <div className="flex justify-between items-center mb-3">
                    ...
                    {result && (
                        <button onClick={() => setIsInsightsPanelOpen(true)} className="relative px-6 py-3 rounded-xl font-semibold text-white bg-gradient-to-r from-fuchsia-500 via-indigo-600 to-violet-600 bg-[length:300%_300%] animate-gradientMove shadow-md hover:shadow-lg group flex items-center gap-2">
                            <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/25 to-transparent -translate-x-[200%] group-hover:translate-x-[200%] transition-transform duration-700"></span>
                ...
                <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
                    ...
                    {result && (
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3 max-h-[70vh] overflow-y-auto pr-2">
                            {result.extracted_sections?.map((s: any, idx: number) => (
        ...
        {/* Correctly configured RightPanel for the Query Page */}
        <RightPanel
            visible={isInsightsPanelOpen}
            onClose={() => setIsInsightsPanelOpen(false)}
            text={insightsTextPayload} // Pass the combined text of all results
            pageType="query" // Use 'query' to show all three insight options
        />
    </div>
  );
};
