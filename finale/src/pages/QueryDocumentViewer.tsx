...
const QueryDocumentViewer: React.FC = () => {
  ...
  // parse ?page=NN if present (used when we navigate to a different PDF and want to jump after load)
  const urlParams = new URLSearchParams(location.search);
  const requestedPageFromUrl = parseInt(urlParams.get('page') || '0', 10) || null;

  // computed current PDF (from route id, selectedDocument, or first pdf)
  const currentPDF = id
    ? getPDFById(id)
    : selectedDocument
  ...
  // set initial selectedDocument from first result
  useEffect(() => {
    if (result?.extracted_sections && !selectedDocument) {
  ...
  // Attempts to get Adobe APIs and gotoLocation with retries.
  // Uses positional args: gotoLocation(pageNumber, x, y)
  const attemptGoto = async (pageNumber: number, retries = 12, delayMs = 300): Promise<boolean> => {
      ...
      try {
        if (viewerRef.current && typeof viewerRef.current.getAPIs === 'function') {
          ...
          if (apis && typeof apis.gotoLocation === 'function') {
            // NOTE: Adobe gotoLocation expects positional args (pageNumber[, x, y])
    ...
    console.warn('attemptGoto: unable to call gotoLocation after retries');
    return false;
  };

  // Initialize Adobe viewer whenever currentPDF or annotated mapping or processing timestamp changes.
  useEffect(() => {
    ...
    adobeDCView
      ...
      .then((viewer: any) => {
        ...
        try {
          ...
          // ignore if registerCallback fails on some builds
        }

        // If a requested page was passed in the URL, attempt to jump to it after the viewer is initialized
        if (requestedPageFromUrl && requestedPageFromUrl > 0) {
          setTimeout(() => {
            attemptGoto(requestedPageFromUrl);
          }, 250);
        }
      })
      .catch((err: any) => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPDF, result?.metadata?.annotated_files, result?.metadata?.processing_timestamp, requestedPageFromUrl]);

  // Handle clicking on a section in the sidebar
  const handleSectionClick = (section: ExtractedSection) => {
    // defensive parse for page number (backend should provide number, but ensure it is number)
    const targetPage = Math.max(1, Math.floor(Number(section.page_number) || 1));
    const targetPDF = getPDFByServerFilename(section.document);
    if (!targetPDF) return;

    // If currently loaded PDF is the same file (or selectedDocument indicates same), try to goto immediately
    const samePdfLoaded =
    ...
    // Different PDF -> navigate route (so viewer will reload) and include ?page=NN so that the viewer attempts to jump after load
    setSelectedDocument(section.document);
    navigate(`/query/${targetPDF.id}?page=${targetPage}`, { state: { result }, replace: true });
    // The viewer initialization useEffect will read the ?page param and attempt the goto after load.
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50 flex">
                ...
                <div key={document} className="space-y-2">
                  ...
                  {sections.map((section, idx) => (
                    <motion.div
                      ...
                      className={`p-3 rounded-lg cursor-pointer transition-all duration-200 border ${
                        selectedDocument === document ? 'bg-primary-100 border-primary-300 shadow-md' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'
                      }`}
                      onClick={() => handleSectionClick(section)}
                    >
                      <div className="flex items-start justify-between">
      ...
      <div className="flex-1 flex flex-col">
        <div className="bg-white shadow-sm border-b border-gray-200 p-4 lg:p-6 relative z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <button onClick={() => navigate('/query')} className="btn-secondary">
                <ArrowLeft className="w-5 h-5 mr-2" />
                Back to Query
              </button>
              {!sidebarOpen && (
            ...
            <div className="flex items-center space-x-4">
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className="hidden lg:flex btn-secondary">
                {sidebarOpen ? (
                  <>
                    <EyeOff className="w-5 h-5 mr-2" />
                    Hide Results
...
//     persona?: string;
//     processing_timestamp?: string;
//     // optional annotated map returned by backend: { original_filename: annotated_filename }
//     annotated_files?: { [original: string]: string };
//   };
// }
...
//   const viewerRef = useRef<any>(null);
//   const location = useLocation();
//   const result: QueryResult | undefined = location.state?.result;
...
//     // Clear previous viewer container
//     const viewerContainer = document.getElementById('pdf-viewer');
...
//     // choose annotated copy if backend created one, else original
//     const annotatedFiles = result?.metadata?.annotated_files || {};
//     const annotatedForThis = annotatedFiles ? annotatedFiles[currentPDF.serverFilename] : null;

//     // cache-bust: prefer processing timestamp from backend (so new annotated file will be fetched)
//     const ts = result?.metadata?.processing_timestamp
//       ? encodeURIComponent(result.metadata.processing_timestamp)
//       : Date.now();
...
//           );
//         } catch (e) {
//           // some builds return the apis directly or have a different object shape — ignore if fails
//           console.debug('registerCallback failed or is not available:', e);
//         }
//       })
//       .catch((err: any) => {
//         console.error('Adobe previewFile error:', err);
...
//       viewerRef.current = null;
//     };
//     // Re-run when the selected PDF changes or a new annotated mapping / timestamp is present
...
//     if (selectedDocument === section.document) {
//       // Same PDF — go to location using the Adobe APIs
//       if (viewerRef.current?.getAPIs) {
//         viewerRef.current
//           .getAPIs()
//           .then((apis: any) => apis.gotoLocation({ pageNumber: section.page_number }))
//           .catch((e: any) => console.error('gotoLocation error:', e));
//       }
//     } else {
//       // Different PDF — navigate so useEffect reloads viewer with new PDF
//       navigate(`/query/${targetPDF.id}`, { state: { result }, replace: true });
//       setSelectedDocument(section.document);

//       // try to jump after a small delay (viewer will re-initialize)
//       setTimeout(() => {
//         if (viewerRef.current?.getAPIs) {
//           viewerRef.current
//             .getAPIs()
//             .then((apis: any) => apis.gotoLocation({ pageNumber: section.page_number }))
//             .catch((e: any) => console.error('gotoLocation error after nav:', e));
//         }
//       }, 1000);
//     }
//   };
