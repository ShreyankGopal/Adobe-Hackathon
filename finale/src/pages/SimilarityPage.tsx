      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-4 h-[calc(100vh-120px)]">
        ...
        <div className="lg:col-span-1 bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border p-4 flex flex-col">
          <div className="flex-shrink-0">
            <h2 className="text-lg font-bold text-slate-800 mb-1">Similarity Analysis</h2>
            <p className="text-xs text-slate-600 mb-3">Similar content found for your selection.</p>
          </div>
          <div className="mb-4">
        ...
        <div className="lg:col-span-3 bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border p-4 flex flex-col">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-xl font-bold text-slate-800">Similarity Results</h3>
            {result?.insights && (
              <button onClick={() => setIsInsightsPanelOpen(true)} className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-4 py-2 rounded-lg">
                AI Insights Hub
              </button>
            )}
          </div>
          <div className="flex-1 overflow-y-auto pr-2">
            {result?.extracted_sections?.length > 0 ? (
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
                {result.extracted_sections.map((s: any, idx: number) => (
                  <div key={`${s.document}-${idx}`} onClick={() => handleExtractedSectionClick(s)}
                    className="bg-gradient-to-br from-white to-slate-50 border rounded-lg p-3 hover:shadow-md cursor-pointer">
              ...
              <div className="text-center text-slate-400 p-12 border-2 border-dashed rounded-lg">No similar sections found.</div>
            )}
          </div>
        </div>
      </div>
...
//     if (location.state?.result) {
//       setResult(location.state.result);
//       setSelectedText(location.state.result.metadata?.selected_text || '');
//     } else {
//       try {
//         const stored = sessionStorage.getItem('pdfQuery.result');
//         const storedText = sessionStorage.getItem('pdfQuery.selectedText') || '';
...
//           <div className="flex-shrink-0">
//             <h2 className="text-lg font-bold text-slate-800 mb-1">Similarity Analysis</h2>
//             <p className="text-xs text-slate-600 mb-3">Finding similar content based on your selection.</p>
//           </div>
...
//           <div className="flex justify-between items-center mb-3">
//             <h3 className="text-xl font-bold text-slate-800 flex-shrink-0">Similarity Results</h3>
//             <button
...
//                       className="bg-white border border-slate-200 rounded-lg p-4 hover:shadow-lg transition-shadow cursor-pointer flex flex-col"
...
//                           </p>
//                           <span className="bg-indigo-100 text-indigo-800 px-2 py-1 rounded-full font-medium text-xs">
//                             Rank #{s.importance_rank}
//                           </span>
//                         </div>
...
// import { motion } from 'framer-motion';
// import { Link, useNavigate, useLocation } from 'react-router-dom';
// import RightPanel from './RightPanel'; // Ensure this component exists
...
//       setSelectedText(location.state.result.metadata?.selected_text || '');
//     } else {
//       // No state passed — try to restore from sessionStorage (in case user navigated earlier)
//       try {
//         const stored = sessionStorage.getItem('pdfQuery.result');
//         const storedText = sessionStorage.getItem('pdfQuery.selectedText') || '';
//         if (stored) {
...
//   const goBackToQuery = () => {
//     // Try to go back in history; if not possible, fallback to QueryDocument route
//     if (window.history.length > 1) {
//       navigate(-1);
//     } else {
//       navigate('/QueryDocument');
...
//           <div className="flex-shrink-0">
//             <h2 className="text-lg font-bold text-slate-800 mb-1">Similarity Analysis</h2>
//             <p className="text-xs text-slate-600 mb-3">Finding similar content</p>
//           </div>
...
//             <button
//               onClick={() => setIsInsightsPanelOpen(true)}
//               className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-4 py-2 rounded-lg shadow-md hover:shadow-lg transition-all hover:scale-[1.03] active:scale-[0.98]"
//             >
//               AI Insights Hub
//             </button>
//           </div>
